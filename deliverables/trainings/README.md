# AI Transformation Enablement — Trainingsunterlagen

## Übersicht

10-wöchiges Remote-Training für Agilisten, Agile Enabler und Transformation Manager.
Ziel: Von "AI-interessiert, aber unsystematisch" zu "kann AI-Tools produktiv nutzen UND eine AI-Transformation beim Kunden strukturiert begleiten."

---

## Dokumente

### Für Programm-Autoren und Trainer-Team

| Dokument | Inhalt |
|---|---|
| [design/konzept.md](design/konzept.md) | Lerndesign, Roter Faden, Gamification, Gast-Trainer-Modell, Teilnehmer als Practice-Experten, Transfer & Messung |
| [design/growth-log.md](design/growth-log.md) | Growth Log: Konzept, Mural-Templates, Keeper-Rotation, Team-Check-in, 🤝 Unterstuetzungs-Feld, spielerischer Kontext |
| [design/self-assessment.md](design/self-assessment.md) | Die 4 Self-Assessment-Fragen (MS Forms-Quelle) + Scoring-Logik |

### Für Trainer (pro Block)

| Block | Titel | Gast? |
|---|---|---|
| [Block 0](blocks/block-00-onboarding.md) | Onboarding & Starter Kit | — |
| [Block 1](blocks/block-01-kickoff.md) | Kickoff — AI als Denkpartner | — |
| [Block 2](blocks/block-02-prompt-engineering.md) | Prompt Engineering Praxis | — |
| [Block 3](blocks/block-03-ai-im-code.md) | AI im Code — GitHub Copilot & Co. | — |
| [Block 4](blocks/block-04-sdlc.md) | AI im SDLC — Von Requirements bis Ops | GAST |
| [Block 5](blocks/block-05-pitfalls.md) | AI-Patterns, Pitfalls & Teamwork | — |
| [Block 6](blocks/block-06-ms-copilot.md) | MS Copilot im Arbeitsalltag | GAST |
| [Block 7](blocks/block-07-craft-framework.md) | Das CRAFT-Framework verstehen | — |
| [Block 8](blocks/block-08-craft-explorer.md) | CRAFT Explorer & Interventionen | GAST |
| [Block 9](blocks/block-09-roadmap.md) | AI Transformation planen | — |
| [Block 10](blocks/block-10-community-day.md) | Community Day | — |

### Für Gast-Trainer

| Dokument | Inhalt |
|---|---|
| [templates/gast-trainer-briefing.md](templates/gast-trainer-briefing.md) | Briefing-Template (ausfüllen + versenden) |

### Für Teilnehmer

| Dokument | Inhalt |
|---|---|
| [teilnehmer-checklist-und-ankuendigung.md](teilnehmer-checklist-und-ankuendigung.md) | Ankündigungs-E-Mail + Transfer-Aufgaben-Übersicht |

---

**Letzte Aktualisierung:** 2026-03-10
